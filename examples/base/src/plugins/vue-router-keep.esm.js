  ...
  render: function render(h) {
    ...
    if (vnode) {
      ...
      if (config.isSingleMode) {
        ...
        var baseKey = routerKeepHelper.resolveKeyFromRoute(this.$route);
...
historyStateEvent.on(BACK, function () {
  ...
  if (config.isSingleMode) {
    var key = routerKeepHelper.resolveKeyFromRoute(route);

    routerKeepHelper._remove(key);
  } else {
    var baseKey = routerKeepHelper.resolveKeyFromRoute(route);

    if (globalMultiKeyMap[baseKey]) {
      var _key = globalMultiKeyMap[baseKey].reduce();

      routerKeepHelper._remove(_key);
    }
  }
});
...
var routerMiddle = function routerMiddle(Vue, config) {
  ...
  router.push = function (location, onComplete, onAbort) {
    routerKeepHelper.removeBackInclue(location);
